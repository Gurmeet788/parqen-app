...
const CARD_TYPES = [
  {
    ...
    name: "E-Code / Digital",
    icon: Laptop,
    description: "Digital code sent via email/SMS",
    color: "#10b981",
    bgColor: "#d1fae5",
  },
  {
    ...
    name: "Physical Card",
    icon: CreditCard,
    description: "Physical gift card will be shipped",
    color: "#f59e0b",
    bgColor: "#fed7aa",
  },
];
    ...
    <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition border border-gray-100 overflow-hidden">
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {margin !== 0 && (
            <div
              className={`text-xs font-bold px-2 py-1 rounded-full ${margin > 0 ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
            >
          ...
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-1">
              {paymentMethods.slice(0, 2).map((method, idx) => {
                const PayIcon = getPaymentIcon(method);
                return (
                  <PayIcon key={idx} size={14} className="text-gray-500" />
                );
              })}
            </div>
          </div>
...
export default function SellGiftCardMarketplace({ user }) {
  ...
  useEffect(() => {
    ...
    const fetchTrades = async () => {
      try {
        const res = await axios.get(`${API_URL}/trades/active`, {
  ...
  const getFilteredOffers = () => {
    ...
    if (filters.minAmount) {
      filtered = filtered.filter(
        (l) =>
          (l.min_amount || l.minAmount || 0) >= parseFloat(filters.minAmount),
      );
    }
    if (filters.maxAmount) {
      filtered = filtered.filter(
        (l) =>
          (l.max_amount || l.maxAmount || 9999) <=
          parseFloat(filters.maxAmount),
      );
    }
    if (filters.minRating) {
      filtered = filtered.filter(
        (l) => (l.users?.average_rating || 0) >= parseFloat(filters.minRating),
      );
    }
    ...
    switch (sortBy) {
      case "best_price":
        filtered.sort(
          (a, b) => (a.bitcoin_price || 0) - (b.bitcoin_price || 0),
        );
        break;
      case "rating":
        filtered.sort(
      ...
      case "trades":
        filtered.sort(
          (a, b) => (b.users?.total_trades || 0) - (a.users?.total_trades || 0),
        );
        break;
      default:
        filtered.sort(
  ...
  if (loading) {
    return (
      <div
        ...
        style={{ backgroundColor: PRAQEN.lightBg }}
      >
        <div className="text-center">
          <div
            className="w-10 h-10 border-4 rounded-full animate-spin mx-auto mb-3"
            style={{
              borderColor: PRAQEN.primary,
              borderTopColor: PRAQEN.secondary,
            }}
          ></div>
          <p className="text-sm" style={{ color: PRAQEN.primary }}>
            Loading buyers...
          </p>
        </div>
      </div>
    ...
    <div className="min-h-screen" style={{ backgroundColor: PRAQEN.lightBg }}>
      <div className="max-w-7xl mx-auto px-3 py-4 md:px-4 md:py-6">
        ...
        <div
          ...
          style={{
            background: `linear-gradient(135deg, ${PRAQEN.primary}, ${PRAQEN.darkBg})`,
          }}
        >
          <div className="p-5">
            ...
            <p className="text-white/70 text-sm">
              Choose a buyer, agree on terms, and get paid in Bitcoin
            </p>
          </div>
        </div>
              ...
              style={{
                borderColor:
                  selectedCardType.id === type.id
                    ...
                    : PRAQEN.gray[200],
                backgroundColor:
                  selectedCardType.id === type.id ? type.bgColor : "white",
              }}
            >
              <div className="flex items-center gap-3">
                <type.icon size={26} style={{ color: type.color }} />
                <div>
                  <p className="font-bold" style={{ color: type.color }}>
                    {type.name}
                  </p>
        ...
        <div className="flex justify-between items-center mb-4">
          <p className="text-sm text-gray-500">
            <span className="font-bold" style={{ color: PRAQEN.primary }}>
              {filteredOffers.length}
            </span>{" "}
            buyers looking for {selectedCardType.name}
          </p>
        </div>
        ...
        {filteredOffers.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            <p className="text-sm text-gray-500 mb-4">
              No one is currently looking to buy {selectedCardType.name}
            </p>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center">
              ...
              <p className="text-xs text-gray-500">
                Browse offers from verified buyers
              </p>
            </div>
            <div className="text-center">
              <div
              ...
              <p className="text-xs text-gray-500">
                Chat and confirm the trade details
              </p>
            </div>
            <div className="text-center">
              <div
              ...
              <p className="text-xs text-gray-500">
                Receive Bitcoin in escrow, then release code
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h2
                ...
                style={{ color: PRAQEN.primary }}
              >
                Sell to {selectedOffer.users?.username}
              </h2>
              <button
            ...
            <div className="space-y-4">
              ...
              >
                <div className="flex justify-between mb-2">
                  ...
                  <span className="font-semibold">
                    1 BTC ≈ ${(selectedOffer.bitcoin_price * 45000).toFixed(2)}{" "}
                    USD
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">
                    {selectedOffer.users?.created_at
                      ? new Date(
                          selectedOffer.users.created_at,
                        ).toLocaleDateString()
                      : "Recently"}
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Min: $
                  {selectedOffer.min_amount || selectedOffer.minAmount || 10} -
                  Max: $
                  {selectedOffer.max_amount || selectedOffer.maxAmount || 5000}
                </p>
              </div>
              ...
              >
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Platform fee: 0.5% (
                  {((parseFloat(tradeAmount) / 45000) * 0.005).toFixed(8)} BTC)
                </p>
              </div>

              <div className="bg-yellow-50 p-3 rounded-lg flex items-start gap-1.5">
                <AlertTriangle
                  size={13}
                  className="flex-shrink-0 mt-0.5 text-yellow-800"
                />
                <p className="text-xs text-yellow-800">
                  Bitcoin will be held in escrow until you provide the gift card
                  code and buyer confirms.
                </p>
              </div>
