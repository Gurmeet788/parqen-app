...
function EditModal({ listing, onClose, onSave, saving, walletBtc, btcPrice }) {
  ...
  const isSell =
    ...
    (listing.listing_type || "").toUpperCase() === "SELL_BITCOIN";

  // Wallet capacity in local currency (only relevant for SELL offers)
  const walletCapLocal = isSell
    ? Math.floor(walletBtc * (btcPrice || 88000) * usdRate)
    : Infinity;
  ...
  const [form, setForm] = useState({
    ...
    min_limit_local:
      ...
      Math.round((listing.min_limit_usd || 10) * usdRate),
    max_limit_local:
      listing.max_limit_local ||
      Math.round((listing.max_limit_usd || 1000) * usdRate),
  ...
  const maxAboveCap =
    isSell && walletCapLocal < Infinity && maxVal > walletCapLocal;
  const maxBelowMin = maxVal > 0 && minVal > 0 && maxVal < minVal;
  const canSave =
    !minBelowFloor && !maxAboveCap && !maxBelowMin && minVal > 0 && maxVal > 0;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-5">
          <div>
              ...
              <button
                onClick={() =>
                  set(
                    "margin",
                    parseFloat(
                      Math.max(-10, parseFloat((form.margin - 0.5).toFixed(1))),
                    ),
                  )
                }
              ...
              <div className="flex-1 relative">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (!isNaN(n))
                      set("margin", Math.max(-10, Math.min(100, n)));
                  }}
                  onBlur={() => {
                    ...
                    if (isNaN(n)) set("margin", 0);
                      ...
                      set(
                        "margin",
                        Math.max(-10, Math.min(100, parseFloat(n.toFixed(1)))),
                      );
                  }}
                ...
                />
                <span
                  className="absolute right-3 top-1/2 -translate-y-1/2 font-black text-base"
                  style={{ color: C.g400 }}
                >
                  %
                </span>
              ...
              <button
                onClick={() =>
                  set(
                    "margin",
                    parseFloat(
                      Math.min(100, parseFloat((form.margin + 0.5).toFixed(1))),
                    ),
                  )
                }
                className="w-11 h-11 rounded-xl border-2 flex items-center justify-center flex-shrink-0"
            ...
            <p
              ...
              style={{ color: C.g400 }}
            >
              Negative = below market rate (attracts more buyers) · Positive =
              your profit above market
            </p>
          </div>
          ...
          <div>
                ...
                return (
                  <div key={key}>
                    <p
                      ...
                      style={{ color: hasErr ? C.danger : C.g500 }}
                    >
                      {label}
                      ...
                      <input
                        ...
                        onBlur={() => {
                          ...
                          if (
                            isMax &&
                            isSell &&
                            walletCapLocal < Infinity &&
                            val > walletCapLocal
                          )
                            set(key, walletCapLocal);
                        }}
                    ...
                    {val > 0 && (
                      <p
                        ...
                        style={{ color: hasErr ? C.danger : C.g400 }}
                      >
            ...
            {(minBelowFloor || maxAboveCap || maxBelowMin) && (
              ...
              >
                <AlertTriangle
                  size={13}
                  style={{ color: C.danger, flexShrink: 0, marginTop: 1 }}
                />
                <p
                ...
                >
                  {minBelowFloor &&
                    `Minimum must be at least ${sym}${fmt(minAllowed)} (= $10 USD). `}
                  {maxAboveCap &&
                    `Maximum cannot exceed your wallet balance of ${sym}${fmt(walletCapLocal)} ${cur}. `}
                  {maxBelowMin &&
          ...
          <div>
            ...
            <div className="grid grid-cols-6 gap-1.5">
              {[15, 30, 45, 60, 90, 120].map((t) => (
                <button
                  key={t}
                  onClick={() => set("time_limit", t)}
                  className="py-2.5 rounded-xl text-sm font-black border-2 transition"
                  style={{
                    borderColor: form.time_limit === t ? C.green : C.g200,
                    backgroundColor:
                      form.time_limit === t ? C.green : "transparent",
                    color: form.time_limit === t ? C.white : C.g500,
                  }}
                >
                  {t}
          ...
          <div>
            ...
            <textarea
              value={form.trade_instructions}
              onChange={(e) => set("trade_instructions", e.target.value)}
              rows={3}
              placeholder="Tell traders exactly how to pay you…"
          ...
          <div>
            ...
            >
              Offer Terms{" "}
              <span className="font-normal text-xs" style={{ color: C.g400 }}>
                (optional)
              </span>
            </label>
            <textarea
              value={form.listing_terms}
              onChange={(e) => set("listing_terms", e.target.value)}
              rows={2}
        ...
        <div
          className="flex-shrink-0 px-5 pt-4 border-t flex gap-3"
          style={{
            borderColor: C.g100,
        ...
        >
          ...
          <button
            onClick={() => canSave && onSave(listing.id, form, usdRate)}
            ...
            style={{
              backgroundColor: canSave ? C.green : C.g400,
              flex: 2,
...
function ToggleSwitch({ active, onToggle, loading }) {
  return (
    ...
    >
      ...
      />
      <div
        className="absolute top-1 transition-all duration-300 w-4 h-4 rounded-full bg-white shadow flex items-center justify-center"
      ...
      >
        {loading ? (
          <RefreshCw
            ...
            style={{ color: active ? C.success : C.g400 }}
          />
        ) : active ? (
...
function OfferCard({ listing, onEdit, onDelete, onToggle, walletBtc }) {
  ...
  const minLocal =
    listing.min_limit_local ||
    (listing.min_limit_usd ? listing.min_limit_usd * usdRate : 0);
  const maxLocal =
    listing.max_limit_local ||
    (listing.max_limit_usd ? listing.max_limit_usd * usdRate : 0);
  const views = parseInt(listing.view_count || 0);

  const isBuyGiftCard =
    (listing.listing_type || "").toUpperCase() === "BUY_GIFT_CARD";
  ...
  return (
    <div
      className="bg-white rounded-xl border transition-all w-full overflow-hidden"
      style={{
        borderColor: isActive ? `${tc.color}40` : C.g200,
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p
            ...
            style={{ color: isActive ? C.success : C.g400 }}
          >
        ...
        <div className="flex flex-col items-center flex-shrink-0">
          ...
          <span
            style={{
              fontSize: 9,
              fontWeight: 900,
              color: isActive ? C.success : C.g400,
            }}
          >
            {isActive ? "ON" : "OFF"}
          </span>
      ...
      >
        ...
        <div className="bg-white px-3 py-2.5">
          ...
          <p className="font-black text-sm mt-0.5" style={{ color: C.g600 }}>
            ...
            />
            {fmt(views)}
            <span className="mx-1" style={{ color: C.g300 }}>
              ·
            </span>
            <Clock size={11} className="inline mr-0.5" />
            {listing.time_limit || 30}m
          </p>
        </div>
      </div>
        ...
        >
          ...
          >
            ...
            <span className="text-xs font-black text-white tracking-wide">
              Offer Paused — Insufficient Balance
            </span>
          </div>
          <div className="px-3 py-2.5" style={{ backgroundColor: "#FFFBEB" }}>
            <p
              ...
              style={{ color: "#92400E" }}
            >
              Your gift card buying offer was automatically paused because your
              PRAQEN wallet has less than <strong>$10 worth of Bitcoin</strong>.
            </p>
            <p
              className="text-xs font-bold mt-1.5"
              style={{ color: "#B45309" }}
            >
              Top up your wallet with at least <strong>$10 in BTC</strong> and
              your offer will be reactivated automatically.
            </p>
          </div>
        </div>
      )}
      ...
      {pausedInactive && (
        <div
          ...
          style={{ border: `2px solid ${C.danger}50` }}
        >
          <div
            className="flex items-center gap-2 px-3 py-2"
            style={{
              background: `linear-gradient(135deg,#991B1B,${C.danger})`,
            }}
          >
            <span className="flex items-center">
            ...
            <span className="text-xs font-black text-white tracking-wide">
              Offer Paused — Inactivity Detected
            </span>
          </div>
          <div className="px-3 py-2.5" style={{ backgroundColor: "#FEF2F2" }}>
            <p
              className="text-xs font-semibold leading-relaxed mb-2"
              style={{ color: "#991B1B" }}
            >
              This offer was automatically paused because your account showed no
              activity for <strong>10+ days</strong>. It is no longer visible to
      ...
      <div className="flex gap-2 p-3">
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-black border transition flex-1"
          style={{
            borderColor: copied ? C.success : C.paid,
            color: copied ? C.success : C.paid,
            backgroundColor: copied ? `${C.success}08` : `${C.paid}06`,
          }}
        >
          {copied ? (
            <>
    ...
    return (
      <div className="text-center py-10">
        <p className="text-3xl mb-2">
          <Mailbox size={48} className="mb-2 text-gray-400 mx-auto" />
        </p>
    ...
    <div className="space-y-2">
      ...
      {filtered.length === 0 ? (
        ...
        >
          ...
          <p className="font-bold text-xs" style={{ color: C.g700 }}>
...
export default function MyListings({ user }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (listingsRes.status === "fulfilled") {
        currentListings = (listingsRes.value.data.listings || []).filter(
          (l) => l.status !== "DELETED",
        );
      } else {
      ...
      if (walletRes.status === "fulfilled") {
        currentWalletBtc = parseFloat(
          walletRes.value.data.available_btc ||
            walletRes.value.data.balance_btc ||
            0,
        );
      ...
      if (currentWalletBtc * liveBtcPrice >= 10) {
        const pausedBtcRequired = currentListings.filter((l) => {
          ...
          return (
            l.status === "PAUSED" &&
            (lt === "SELL" || lt === "SELL_BITCOIN" || lt === "BUY_GIFT_CARD")
          );
        });
        if (pausedBtcRequired.length > 0) {
          ...
          currentListings = currentListings.map((l) =>
            pausedBtcRequired.some((p) => p.id === l.id)
              ? { ...l, status: "ACTIVE" }
              : l,
          );
        }
  ...
  const deleteListing = async (id) => {
    ...
    try {
      ...
      if (removed)
        setListings((prev) => [removed, ...prev.filter((l) => l.id !== id)]);
  ...
  const toggleStatus = async (id, current) => {
    ...
    try {
      ...
      if (!r.data.success) {
        ...
        toast.success(
          `Offer ${next === "ACTIVE" ? "activated ✅" : "paused ⏸"}`,
        );
      }
    } catch {
      setListings((prev) =>
  ...
  const toggleAll = async (ids, targetStatus) => {
    setListings((prev) =>
      prev.map((l) =>
        ids.includes(l.id) ? { ...l, status: targetStatus } : l,
      ),
    );
    ...
    if (failed === 0)
      toast.success(
        `All offers ${targetStatus === "ACTIVE" ? "activated ✅" : "paused ⏸"}`,
      );
    else {
      toast.warn(`${failed} offer(s) failed to update`);
      load();
    }
  };
      ...
      <div className="max-w-3xl mx-auto w-full px-3 sm:px-4 py-5 space-y-4">
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className="text-xs mt-0.5" style={{ color: C.g500 }}>
              {listings.length} total · {totalActive} active · {totalViews}{" "}
              views
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          ...
          <StatCard
            ...
            color={C.gold}
            sub={`${buyListings.length} buy · ${gcListings.length} gift`}
          />
        </div>
        ...
        {listings.length > 0 && (
          <div
            className="flex items-center justify-between p-3 rounded-xl border flex-wrap gap-2"
            style={{
              backgroundColor: allAreActive ? `${C.warn}08` : `${C.success}08`,
              borderColor: allAreActive ? `${C.warn}30` : `${C.success}30`,
            }}
          >
            <div className="min-w-0">
              <p className="text-xs font-black" style={{ color: C.forest }}>
                {allAreActive
                  ? "⚡ All offers are live"
                  : totalActive === 0
                    ? "⏸ All offers are paused"
                    : `${totalActive} of ${listings.length} offers active`}
              </p>
              <p className="text-xs mt-0.5" style={{ color: C.g500 }}>
                Toggle all offers at once
              </p>
            </div>
        ...
        {listings.length === 0 && (
          ...
          >
            ...
            <p className="text-sm mb-5" style={{ color: C.g500 }}>
              Create your first offer to start earning on PRAQEN.
            </p>
            <button
              onClick={() => navigate("/create-offer")}
        ...
        {listings.length > 0 && (
          <>
            {/* ── TABS ── */}
            <div
              className="bg-white rounded-2xl border overflow-hidden w-full"
              style={{ borderColor: C.g200 }}
            >
              {/* Tab bar */}
                ...
                {TABS.map((tab) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {tab.count > 0 && (
                        <span
                          className="font-black px-1.5 py-0.5 rounded-full flex-shrink-0"
                          style={{
                            backgroundColor: isCur ? tab.color : C.g100,
                            color: isCur ? "#fff" : C.g500,
                            fontSize: 10,
                          }}
                        >
                          {tab.count}
              ...
              >
                <p
                  className="text-xs font-semibold flex-1 hidden sm:block"
                  style={{ color: C.g500 }}
                >
                  {activeTab === "sell" &&
                    "Listed on the Buy Bitcoin marketplace page"}
                  {activeTab === "buy" &&
                    "Listed on the Sell Bitcoin marketplace page"}
                  {activeTab === "gift" &&
                    "Listed on the Gift Cards marketplace page"}
                </p>
            ...
            {/* ── CREATE MORE CTA ── */}
            <div
              className="flex items-center justify-between p-3.5 rounded-xl border flex-wrap gap-2"
              style={{
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-xs" style={{ color: C.g500 }}>
                  Create another offer to reach more buyers and sellers.
                </p>
              </div>
              <button
                onClick={() => navigate("/create-offer")}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {[
                {
                  icon: "💡",
                  title: "Competitive margin gets more trades",
                  desc: "Offers within ±5% of market rate receive 3× more trade requests.",
                },
                {
                  icon: "⚡",
                  title: "Keep offers active",
                  desc: "Paused offers disappear from the marketplace. Activate to stay visible.",
                },
                {
                  icon: "🔗",
                  title: "Share your offer link",
                  desc: "Send your offer link directly to buyers or sellers to skip the marketplace queue.",
                },
              ].map(({ icon, title, desc }) => (
                <div
                  key={title}
