...
function WithdrawModal({ balance, btcPrice, onClose, onSend, kycStatus }) {
  ...
  const calcFeeByUsd = (usd) => {
    ...
    if (usd < 100)
      return { feeUsd: 10, feeBtc: 10 / price, label: "$10 flat fee" };
    if (usd < 250)
      return { feeUsd: 15, feeBtc: 15 / price, label: "$15 flat fee" };
    if (usd < 500)
      return { feeUsd: 25, feeBtc: 25 / price, label: "$25 flat fee" };
    return {
      feeUsd: usd * 0.05,
      feeBtc: (usd * 0.05) / price,
      label: "5% fee",
    };
  };
  const calcFee = (btc) => {
  ...
  const requestCode = async () => {
    ...
    try {
      const t = localStorage.getItem("token");
      await axios.post(
        `${API_URL}/auth/send-action-code`,
        { action: "send_btc" },
        { headers: t ? { Authorization: `Bearer ${t}` } : {} },
      );
      setCodeInput("");
      setStep("code");
      toast.success("Security code sent! Check your email inbox.", {
        autoClose: 5000,
      });
    } catch (e) {
      toast.error(
        e?.response?.data?.error ||
          "Could not send security code. Please try again.",
      );
    } finally {
      setSending2FA(false);
    }
  ...
  const handleSend = async () => {
    ...
    try {
      if (riskyAttempt) {
        toast.info(
          "Since you still want to send, you can proceed. Once sent, PRAQEN is not responsible for any loss.",
          { autoClose: 7000 },
        );
      }
      await onSend(address.trim(), btcAmt, codeInput, riskyAttempt);
      toast.success("BTC Sent Successfully! Your transaction is on its way.", {
        autoClose: 6000,
      });
      onClose();
    } catch (e) {
      ...
      if (
        msg.toLowerCase().includes("risky") ||
        msg.toLowerCase().includes("blockchain issue")
      ) {
        ...
        toast.error(
          "Something went wrong. Your funds are safe — please try again.",
        );
      }
    } finally {
      setSending(false);
    ...
    >
      ...
      >
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                >
                  On-chain · ~10 min · blockchain fee applies
                </p>
              </div>
            </div>
            <button
        ...
        <div className="p-5 overflow-y-auto" style={{ maxHeight: "75vh" }}>
          {/* ── KYC gate ── */}
          {kycStatus &&
          !(kycStatus.email && kycStatus.phone && kycStatus.kyc) ? (
                ...
                {[
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{ color: done ? C.success : C.g700 }}
                    >
                      {label}
                    </p>
                    {done ? (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {address.length > 5 && !addrOk && (
                  <p
                    className="text-xs mt-1.5 font-semibold flex items-center gap-1"
                    style={{ color: "#ef4444" }}
                  >
                    <AlertTriangle size={11} /> Mainnet only — bc1…, 1…, or 3…
                    Testnet not accepted.
                  </p>
                )}
                {addrOk && (
                  <p
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div
                    ...
                    style={{ border: `1.5px solid ${C.g200}` }}
                  >
                    {["btc", "usd"].map((mode) => (
                      <button
                        key={mode}
                ...
                {inputMode === "btc" ? (
                  <div>
                    <div className="relative">
                      <span
                        className="absolute left-4 top-1/2 -translate-y-1/2 font-black text-base"
                        style={{ color: C.g400 }}
                      >
                        ₿
                      </span>
                      ...
                      <button
                        onClick={() =>
                          setAmount(
                            (parseFloat(balance || 0) * 0.999).toFixed(8),
                          )
                        }
                ...
                {btcAmt > 0 && !hasEnough && (
                  <div
                    className="flex items-center gap-1.5 mt-2 px-3 py-2 rounded-xl"
                    style={{
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{ color: "#dc2626" }}
                    >
                      Insufficient balance — need ₿ {fmt(total)} (
                      {fmtUsdVal(totalUsd)}) incl. fee
                    </p>
                  </div>
                )}
              </div>
                  ...
                  {[
                    ...
                    >
                      <span
                        ...
                        style={{ color: bold ? C.g700 : C.g500 }}
                      >
                        {icon && <span>{icon}</span>}
                        {label}
                      </span>
                      <div className="text-right">
                        <span
                          className={`text-xs ${bold ? "font-black" : "font-bold"}`}
                          style={{ color: bold ? "#1e293b" : C.g700 }}
                        >
                          ₿ {fmt(btc)}
                        </span>
                        <span
              ...
              {/* ── Confirm checkbox ── */}
              <label
                className="flex items-start gap-3 cursor-pointer p-3 rounded-2xl transition"
                style={{
                  backgroundColor: confirm ? "#f0fdf4" : "#fafafa",
                  border: `1.5px solid ${confirm ? "#bbf7d0" : C.g200}`,
                }}
              >
                <div
                  className="mt-0.5 w-4 h-4 rounded-md flex items-center justify-center flex-shrink-0"
                  style={{
                    backgroundColor: confirm ? "#10b981" : "#fff",
                    border: `2px solid ${confirm ? "#10b981" : C.g300}`,
                  }}
                >
                  {confirm && (
                ...
                <p
                  ...
                  style={{ color: confirm ? "#166534" : C.g600 }}
                >
                  I confirm this address is correct. Bitcoin transactions are
              ...
              {step === "form" && (
                <div className="space-y-3">
                  <button
                    onClick={requestCode}
                    disabled={!valid || !confirm || sending2FA}
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{ color: "#92400e" }}
                    >
                      A 6-digit security code will be emailed to you to confirm
                      this withdrawal.
                    </p>
                  </div>
                </div>
              ...
              {step === "code" && (
                <div className="space-y-3">
                  <div
                    className="flex flex-col items-center text-center px-4 py-4 rounded-2xl"
                    style={{
                      background: "linear-gradient(135deg, #f0fdf4, #dcfce7)",
                      border: "1px solid #bbf7d0",
                    }}
                  >
                    ...
                    <p className="text-xs mt-0.5" style={{ color: "#15803d" }}>
                      Enter the 6-digit code emailed to you to confirm sending ₿
                      {fmt(btcAmt)}
                    </p>
                  </div>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      backgroundColor:
                        codeInput.length === 6 ? "#f0fdf4" : "#fafafa",
                      color: C.g800,
                      letterSpacing: "0.5em",
                    }}
                    maxLength={6}
      ...
      <div className="bg-white w-full md:max-w-sm rounded-t-2xl md:rounded-2xl overflow-hidden shadow-2xl">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-black text-sm" style={{ color: C.g800 }}>
              Deposit Bitcoin
            </h2>
          </div>
          <button
            onClick={onClose}
            className="w-7 h-7 rounded-xl flex items-center justify-center hover:bg-gray-100"
          >
          ...
          {address ? (
            <>
              <p className="text-xs text-gray-500">
                Send BTC to your unique address. Credited after 1 confirmation
                (~10 min).
              </p>
              ...
              <div>
                ...
                <div
                  className="p-3 rounded-xl border font-mono text-xs break-all"
                  style={{
                    borderColor: C.g200,
              ...
              >
                {checking ? (
                  ...
                  <>
                    <Zap size={12} style={{ color: C.gold }} /> Check for New
                    Deposits
                  </>
                )}
              </button>
              ...
              >
                ...
                <p className="text-xs font-semibold text-blue-700">
                  Only send Bitcoin (BTC) to this address. Other coins will be
                  lost permanently.
                </p>
              </div>
            </>
            ...
            <div className="text-center py-6">
              ...
              >
                <Bitcoin size={14} className="inline mr-1.5" /> Generate My
                Address
              </button>
            </div>
          )}
...
function TxReceiptModal({ tx, onClose, onRepeat, btcPrice }) {
  ...
  const counterpartMatch =
    isInternal && tx.notes
      ? tx.notes.match(/from @(\S+)/i) ||
        tx.notes.match(/→ @(\S+)/i) ||
        tx.notes.match(/to @(\S+)/i)
      : null;
  const counterpart = counterpartMatch
    ? counterpartMatch[1].replace(/\s*[·\-].*$/, "").trim()
    : null;
  ...
  const walletType = isInternal
    ? "PRAQEN Internal Transfer"
    : isTrade
      ? "PRAQEN Escrow"
      : "On-chain Bitcoin";
  ...
  const price = btcPrice || 88000;
  const amountUsd = (amountBtc * price).toLocaleString("en-US", {
    style: "currency",
    currency: "USD",
  ...
  const rows = [
    ...
    {
      label: "Amount",
      value: `${isSend ? "−" : "+"}₿${fmt(amountBtc)} (≈ ${amountUsd})`,
      colored: true,
    },
    tx.fee_btc ? { label: "Fee", value: `₿${fmt(tx.fee_btc)}` } : null,
    {
      label: "Status",
      value: isPending ? "Pending" : "Confirmed",
      statusBadge: true,
    },
    { label: "Date & Time", value: fullDate },
    tx.to_address
      ? { label: "To Address", value: tx.to_address, mono: true }
      : null,
    tx.from_address
      ? { label: "From Address", value: tx.from_address, mono: true }
      : null,
    txHash
      ? {
          label: "TX Hash",
          value: txHash,
          mono: true,
          link: isInternal ? null : `https://mempool.space/tx/${txHash}`,
        }
      : null,
    tx.notes
      ? { label: "Notes", value: normalizeNotes(tx.notes), isNotes: true }
      : null,
      ...
      <div className="bg-white w-full md:max-w-sm rounded-t-3xl md:rounded-3xl overflow-hidden shadow-2xl">
        ...
        >
          <div className="px-5 pt-5 pb-10 text-center">
            ...
            <p className="text-white/50 text-xs font-black tracking-widest mb-0.5">
              PRAQEN
            </p>
            ...
            <div
              className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mt-4"
              style={{
            ...
            <span
              className="text-xs font-black px-3 py-1 rounded-full mt-2 inline-flex items-center gap-1.5"
              style={{
                backgroundColor: isPending
                  ? "rgba(245,158,11,0.3)"
                  : "rgba(16,185,129,0.3)",
        ...
        >
          <div
            className="border-t-2 border-dashed mb-3"
            style={{ borderColor: C.g200 }}
          />
          {rows.map(
            ({ label, value, colored, mono, link, statusBadge, isNotes }) => {
              if (isNotes) {
                    ...
                    {isRisky ? (
                      ...
                      >
                        ...
                        >
                          <AlertTriangle
                            size={13}
                            style={{ color: C.warn, flexShrink: 0 }}
                          />
                          <p
                            className="text-xs font-black"
                            style={{ color: C.warn }}
                          >
                            Risky Wallet Warning
                          </p>
                        </div>
                        {/* Bullet points */}
                        <div
                        ...
                        >
                          {[
                            {
                              icon: CircleCheck,
                              text: "User confirmed twice before sending",
                            },
                            {
                              icon: TriangleAlert,
                              text: "Warned this is a risky wallet and chose to proceed",
                            },
                            feeAmt
                              ? {
                                  icon: Wallet,
                                  text: `Fee of ₿${feeAmt} held by PRAQEN`,
                                }
                              : null,
                            {
                              icon: ShieldX,
                              text: "PRAQEN is not responsible for any loss from this transaction",
                            },
                          ]
                            .filter(Boolean)
                            .map(({ icon: Icon, text }) => (
              ...
              return (
                ...
                >
                  <p
                    className="text-xs font-bold flex-shrink-0 mr-4"
                    style={{ color: C.g400 }}
                  >
                    {label}
                  </p>
                  {link ? (
                    ...
                    <p
                      className={`text-xs text-right break-all ${mono ? "font-mono" : "font-semibold"}`}
                      style={{
                        color: colored ? color : C.g700,
                        maxWidth: "60%",
                      }}
                    >
                      {mono && value.length > 22
                        ? `${value.slice(0, 22)}…`
                        : value}
                    </p>
                  )}
          ...
          {isOnChain && (
            <div className="border-t-2 border-dashed mt-3 pt-3 text-center">
              ...
              <p
                className="text-xs mt-1 font-semibold flex items-start justify-center gap-1"
                style={{ color: C.warn }}
              >
                <TriangleAlert size={14} className="shrink-0 mt-0.5" />
                <span>
                  The blockchain network is responsible for this external wallet
                  transaction. PRAQEN is not liable once funds leave to an
                  external address.
                </span>
              </p>
            </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {isInternal && (
            <span
              ...
              style={{ backgroundColor: `${C.paid}15`, color: C.paid }}
            >
              FREE
            </span>
          )}
        ...
        <div className="flex items-center gap-1">
          <p className="text-xs truncate" style={{ color: C.g400 }}>
            {normalizeNotes(tx.notes) ||
              (txHash ? `${txHash.slice(0, 14)}…` : fmtAge(tx.created_at))}
          </p>
          {txHash && !isInternal && (
            <a
...
function InternalTransferModal({
  balance,
  btcPrice,
  displayCurrency,
  fxRate,
  ...
  const btcAmount =
    localNum > 0 ? parseFloat((localNum / btcPriceLoc).toFixed(8)) : 0;
  ...
  const findUser = async () => {
    ...
    try {
      if (inputMode === "username") {
        const r = await axios.get(`${API_URL}/users/${encodeURIComponent(q)}`, {
          headers: authH(),
        });
        const u = r.data?.user || r.data;
        if (!u?.id) {
          setLookupError("Username not found on PRAQEN.");
          return;
        }
  ...
  const send = async () => {
    ...
    try {
      ...
      const err =
        e.response?.data?.error || "Transfer failed. Please try again.";
      toast.error(err, { autoClose: 8000 });
      if (e.response?.data?.isExternal) {
        setLookupError(
          "This is an external address — use the Send (on-chain) button instead.",
        );
        setStep("form");
      }
    } finally {
        ...
        <div className="overflow-y-auto" style={{ maxHeight: "75vh" }}>
          ...
          {step === "success" && (
            <div className="p-8 text-center space-y-5">
              ...
              <div>
                <p className="font-black text-2xl" style={{ color: C.forest }}>
                  Sent!
                </p>
                <p
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {currentUser?.avatar_url ? (
                    ...
                    <div
                      className="w-11 h-11 rounded-2xl flex items-center justify-center font-black text-white text-base"
                      style={{
              ...
              >
                {[
                  {
                    label: "Amount",
                    val: `${sym}${localNum.toLocaleString("en-US", { minimumFractionDigits: 2 })} ${displayCurrency}`,
                  },
                  { label: "In BTC", val: `₿ ${btcAmount.toFixed(8)}` },
                  { label: "Network fee", free: true },
                  {
                    label: "They receive",
                    val: `₿ ${btcAmount.toFixed(8)}`,
                    bold: true,
                  },
                  ...
                  >
                    ...
                    {free ? (
                      ...
                      <span
                        className={`text-xs ${bold ? "font-black" : "font-bold"}`}
                        style={{ color: bold ? C.forest : C.g700 }}
                      >
                        {val}
                      </span>
                    )}
              ...
              >
                ...
                <p
                  ...
                  style={{ color: "#92400e" }}
                >
                  This transfer is instant and irreversible. Double-check the
                  recipient.
                </p>
              </div>
            ...
            <div className="p-5 space-y-4">
              ...
              >
                ...
                <div className="text-right">
                  ...
                  >
                    {sym}
                    {(parseFloat(balance) * btcPriceLoc).toLocaleString(
                      "en-US",
              ...
              <div>
                ...
                {localNum > 0 && !hasEnough && (
                  <div
                    className="flex items-center gap-1.5 mt-1.5 px-3 py-2 rounded-xl"
                    style={{
                      backgroundColor: "#fff5f5",
                      border: "1px solid #fecaca",
                    }}
              ...
              <button
                ...
                style={{
                  background:
                    !recipient || !hasEnough || btcAmount <= 0
                      ? "#94a3b8"
                      : "linear-gradient(135deg, #6366f1, #4f46e5)",
                  boxShadow:
                    !recipient || !hasEnough || btcAmount <= 0
...
function UsdtWithdrawModal({ balance, onClose, onSend, kycStatus }) {
  ...
  const calcFee = (amt) =>
    amt <= 50 ? FEE_FLAT : parseFloat((amt * FEE_PERCENT).toFixed(2));
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      toast.success("USDT sent! Transaction broadcast to Tron network.", {
        autoClose: 6000,
      });
      onClose();
    } catch (e) {
      const msg =
        e?.response?.data?.error ||
        "Send failed. Your funds are safe — please try again.";
      setSendError(msg);
    } finally {
      setSending(false);
    }
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div
                    ...
                    className="flex items-center gap-3 p-3 rounded-2xl"
                    style={{
                      backgroundColor: done ? "#10b98108" : "#f59e0b08",
                      border: `1px solid ${done ? "#10b98130" : "#f59e0b30"}`,
                    }}
                  >
                    <div
                      className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0"
                      style={{
                        backgroundColor: done ? "#10b98120" : "#f59e0b20",
                      }}
                    >
                      {done ? (
                    ...
                    <p
                      className="text-sm font-bold flex-1"
                      style={{ color: done ? "#10b981" : "#374151" }}
                    >
                      {label}
                    </p>
                    {done ? (
            ...
            <>
              ...
              <div>
                ...
                {address.length > 3 && !addrOk && (
                  <p
                    ...
                    style={{ color: "#ef4444" }}
                  >
                    <AlertTriangle size={11} /> Must start with T, 34 characters
                    (Tron mainnet)
                  </p>
                )}
                {addrOk && (
                  <p
                    ...
                    style={{ color: "#10b981" }}
                  >
                    <CheckCircle size={11} /> Valid Tron address
                  </p>
                )}
              </div>
              <div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    {parseFloat(usdInput) > 0 && (
                      <p
                        ...
                        style={{ color: "#2563eb" }}
                      >
                        = ₮{parseFloat(usdInput || 0).toFixed(2)} USDT · Min:
                        $5.00 · Max: ${calcMax(bal).toFixed(2)}
                      </p>
                    )}
                    {!usdInput && (
                      <p
                        ...
                        style={{ color: C.g400 }}
                      >
                        Min: $5.00 · Max: ${calcMax(bal).toFixed(2)} · 1 USD = 1
                        USDT
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    {usdtAmt > 0 && (
                      <p
                        ...
                        style={{ color: "#10b981" }}
                      >
                        = ${usdtAmt.toFixed(2)} USD · Min: ₮
                        {MIN_SEND.toFixed(2)} · Max: ₮{calcMax(bal).toFixed(2)}
                      </p>
                    )}
                    {!amount && (
                      <p
                        ...
                        style={{ color: C.g400 }}
                      >
                        Min: ₮{MIN_SEND.toFixed(2)} · Max: ₮
                        {calcMax(bal).toFixed(2)} · 1 USDT = $1.00
                      </p>
                    )}
                  </div>
                )}
                ...
                {usdtAmt > 0 && !hasEnough && (
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{ color: "#dc2626" }}
                    >
                      Need ₮{totalDeduct.toFixed(2)} (₮{usdtAmt.toFixed(2)} +{" "}
                      {fee.toFixed(2)} fee) — only ₮{bal.toFixed(2)} available
                    </p>
                  </div>
                )}
              </div>
              ...
              {usdtAmt > 0 && (
                ...
                >
                  {[
                    {
                      label: "You send",
                      val: `₮${usdtAmt.toFixed(2)}`,
                      icon: "→",
                      bold: false,
                    },
                    {
                      label: `Platform fee (${usdtAmt <= 50 ? "₮4 flat" : "4%"})`,
                      val: `₮${fee.toFixed(2)}`,
                      icon: "💰",
                      bold: false,
                      warn: true,
                  ...
                  >
                    ...
                    >
                      ...
                      <strong>₮{usdtAmt.toFixed(2)}</strong> · fee is separate
                    </p>
                  </div>
                </div>
              )}
              ...
              >
                ...
                <p
                  ...
                  style={{ color: confirm ? "#166534" : C.g600 }}
                >
                  I confirm this Tron address is correct. USDT-TRC20
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{ color: "#92400e" }}
                    >
                      A 6-digit security code will be emailed to confirm this
                      withdrawal.
                    </p>
                  </div>
                </div>
              ...
              {step === "code" && (
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <p className="text-xs mt-0.5" style={{ color: "#15803d" }}>
                      Enter the 6-digit code to confirm this withdrawal
                    </p>
                  </div>
                  ...
                  {sendError && (
                    ...
                    >
                      ...
                      <p className="text-xs mt-1" style={{ color: "#b91c1c" }}>
                        Your balance has been fully restored — no funds were
                        lost.
                      </p>
                    </div>
                  )}
...
function UsdtInternalTransferModal({ balance, onClose, onTransfer }) {
  ...
  const handleRecipientContinue = async () => {
    ...
    if (!raw) {
      setErr("Enter a PRAQEN username");
      return;
    }
    ...
    try {
      ...
      const u = r.data?.user || r.data;
      if (!u?.username) {
        setErr("User not found on PRAQEN");
        return;
      }
  ...
  const handleAmountContinue = () => {
    ...
    if (amt < 0.01) {
      setErr("Minimum transfer is ₮0.01 USDT");
      return;
    }
    if (amt > bal) {
      setErr(`Insufficient balance. Available: ₮${bal.toFixed(2)}`);
      return;
    }
    setErr("");
    setStep("preview");
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{
            background: `linear-gradient(145deg, ${C.forest} 0%, ${C.green} 60%, #26A17B 100%)`,
          }}
        >
          {/* decorative circle */}
          <div
              ...
              {["recipient", "amount", "preview"].map((s, i) => (
                <div
                  ...
                  style={{
                    ...
                    height: 6,
                    backgroundColor:
                      ["recipient", "amount", "preview", "success"].indexOf(
                        step,
                      ) >= i
            ...
            <>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <span
                      className="absolute left-4 top-1/2 -translate-y-1/2 font-black text-sm select-none"
                      style={{ color: C.g400 }}
                    >
                      @
                    </span>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: `0 4px 14px ${C.forest}40`,
                    }}
                  >
                    {searching ? (
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(135deg, ${C.mist}, #f0fdf8)`,
              ...
              >
                {[
                  {
                    icon: Zap,
                    text: "Instant settlement — no blockchain delay",
                  },
                  {
                    icon: BadgeCheck,
                    text: "Zero fees — completely free between PRAQEN users",
                  },
                  {
                    icon: ShieldCheck,
                    text: "Username search only — use Send (on-chain) for external wallets",
                  },
                ].map(({ icon: Icon, text }) => (
            ...
            <>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {recipient.full_name && (
                    <p className="text-xs truncate" style={{ color: C.green }}>
                      {recipient.full_name}
                    </p>
                  )}
                  {recipient.isTronAddr && (
              ...
              <div>
                ...
                >
                  <div className="flex items-center gap-3 px-4 py-4">
                    ...
                    <div className="flex-1">
                      <input
                        ...
                        onKeyDown={(e) =>
                          e.key === "Enter" && handleAmountContinue()
                        }
                        placeholder="0.00"
                        step="0.01"
                        min="0.01"
                  ...
                  <div
                    ...
                    style={{
                      background: `linear-gradient(135deg, ${C.mist}60, #f0fdf8)`,
                ...
                {amt > 0 && amt <= bal && (
                  <div
                    className="mt-2 flex items-center justify-between px-3.5 py-2.5 rounded-xl"
                    style={{
                  ...
                  >
                    ...
                    <span
                      className="text-xs font-black"
                      style={{
                        color: remaining < bal * 0.1 ? "#f59e0b" : C.green,
                      }}
                    >
                ...
                {/* Fee note */}
                <div
                  className="mt-2 flex items-center justify-between px-3.5 py-2"
                  style={{
                    background: "#f0fdf4",
                    border: "1px solid #bbf7d0",
                    borderRadius: 12,
                ...
                <button
                  ...
                  style={{
                    background:
                      !amount || amt <= 0
                        ? "#94a3b8"
                        : `linear-gradient(135deg, ${C.forest}, ${C.green})`,
                    boxShadow:
                      amount && amt > 0 ? `0 6px 20px ${C.forest}45` : "none",
                  }}
                >
                  Preview Transfer →
                </button>
          ...
          {step === "preview" && recipient && (
            <>
              <div className="text-center pb-1">
                <p className="font-black text-base" style={{ color: C.g800 }}>
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(145deg, ${C.forest} 0%, ${C.green} 100%)`,
                }}
              >
                <div
                ...
                <div
                  className="items-center justify-center gap-1.5 mt-3 px-3 py-1.5 rounded-full inline-flex mx-auto"
                  style={{
              ...
              >
                {[
                  {
                    label: "Recipient",
                    val: recipient.displayName,
                    mono: false,
                  },
                  {
                    label: "Network",
                    val: "PRAQEN Internal (TRC-20)",
                    mono: false,
                  },
                  {
                    label: "Amount (USDT)",
                  ...
                  {
                    label: "You will have",
                    val: `₮ ${remaining.toFixed(2)} USDT  ·  $ ${remaining.toFixed(2)}`,
                    mono: false,
                  },
              ...
              {recipient.isTronAddr && (
                ...
                >
                  <span className="text-yellow-500 text-sm flex-shrink-0 mt-0.5">
                    ⚠
                  </span>
                  <p
                    className="text-xs font-semibold"
                    style={{ color: "#92400e" }}
                  >
                    Sending to a Tron address. This is an internal PRAQEN
                    transfer — the recipient must be a PRAQEN user. Transfers
                    cannot be reversed.
                  </p>
                </div>
              )}
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button
                  ...
                  style={{
                    background: sending
                      ? "#94a3b8"
                      : `linear-gradient(135deg, ${C.forest} 0%, ${C.green} 100%)`,
                ...
                >
                  {sending ? (
                    ...
                    <>
                      <ArrowLeftRight size={15} /> Confirm & Send ₮
                      {amt.toFixed(2)}
                    </>
                  )}
                </button>
            ...
            <div className="text-center py-3 space-y-5">
              ...
              <div className="relative mx-auto w-24 h-24">
                <div
                  ...
                  style={{
                    background: `linear-gradient(135deg, ${C.forest} 0%, ${C.green} 60%, #26A17B 100%)`,
                    boxShadow: `0 16px 48px ${C.forest}55`,
                  }}
                >
              ...
              <div
                ...
                style={{ border: `1.5px solid ${C.sage}40` }}
              >
                <div
                  className="px-4 py-3"
                  style={{
                    background: `linear-gradient(135deg, ${C.mist}, #f0fdf8)`,
                    borderBottom: `1px solid ${C.sage}25`,
                  }}
                >
                  <p
                ...
                {[
                  {
                    label: "Sent to",
                    val: `${result.to ? "@" : ""}${result.to || recipient.displayName}`,
                  },
                  {
                    label: "Amount USDT",
                    val: `₮ ${parseFloat(result.amount_usdt).toFixed(2)}`,
                  },
                  {
                    label: "Amount USD",
                    val: `$ ${parseFloat(result.amount_usdt).toFixed(2)}`,
                  ...
                  {
                    label: "New balance",
                    val: `₮ ${parseFloat(result.new_balance).toFixed(2)} USDT`,
                  },
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{
                        color: C.forest,
                        fontFamily:
                          label === "Reference" ? "monospace" : "inherit",
...
function TetherGlyph({ size = 16, color = "#fff" }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
      <path
        d="M17.6 15.73v-2.05h4.35V10.5H10.07v3.18H14.4v2.05C10.6 15.9 7.8 16.72 7.8 17.7s2.8 1.8 6.6 1.97v7.03h3.2v-7.03c3.8-.17 6.6-.99 6.6-1.97s-2.8-1.8-6.6-1.97zm0 3.32c-.18.01-.62.04-1.62.04-.86 0-1.46-.02-1.67-.04v.01c-2.89-.13-5.05-.63-5.05-1.25s2.16-1.11 5.05-1.24v1.97c.21.01.82.05 1.68.05.98 0 1.45-.04 1.61-.05v-1.97c2.9.13 5.06.63 5.06 1.24s-2.16 1.12-5.06 1.25v-.01z"
        fill={color}
      />
    </svg>
  );
  ...
  const options = [
    {
      ...
      label: "Bitcoin",
      sub: "BTC Network",
      bg: "linear-gradient(135deg,#F7931A,#e8830a)",
      icon: (
    ...
    <div className="relative">
      <button
        ...
        style={{
          ...
          boxShadow:
            pulse && !open ? "0 0 0 3px rgba(38,161,123,0.45)" : "none",
        }}
      >
        {pulse && !open && (
          <span
        ...
        <ChevronDown
          size={12}
          style={{
            color: dark ? "rgba(255,255,255,0.7)" : C.g500,
      ...
      {open && (
        <>
          ...
          <div
            className="absolute left-0 top-full mt-2 w-48 rounded-2xl shadow-xl overflow-hidden z-20"
          ...
          >
            {options.map((opt) => (
              <button
                ...
                style={{
                  backgroundColor:
                    activeCoin === opt.key ? C.mist : "transparent",
                }}
              >
                <div
                  className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0"
...
function AssetPickerSheet({ title, subtitle, onPick, onClose }) {
  return (
    ...
    >
      <div
        className="bg-white w-full md:max-w-xs rounded-t-3xl md:rounded-3xl overflow-hidden shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
      ...
      <div className="bg-white w-full md:max-w-sm rounded-t-2xl md:rounded-2xl overflow-hidden shadow-2xl">
        ...
        >
          <div className="flex items-center gap-2">
            <div
              className="w-8 h-8 rounded-xl flex items-center justify-center"
              style={{ backgroundColor: "#26A17B15" }}
            >
        ...
        <div className="p-5 space-y-4">
          <p className="text-xs text-gray-500">
            Send USDT (TRC-20) to your Tron address. Credited after network
            confirmation.
          </p>

          <div>
            <label className="block text-xs font-bold mb-1.5 text-gray-600">
              Your Tron (USDT-TRC20) Address
            </label>
            <div
              className="p-3 rounded-xl border font-mono text-xs break-all"
              style={{
          ...
          {address && (
            <button
              onClick={onCheckDeposits}
              disabled={checking || scanCooldown > 0}
            ...
            >
              {checking ? (
                ...
                <>
                  ...
                  {scanCooldown > 0
                    ? `Scan available in ${scanCooldown}s`
          ...
          >
            ...
            <p className="text-xs font-semibold" style={{ color: "#92400e" }}>
              Only send <strong>USDT TRC-20</strong> to this address. Other
              coins or networks will be lost permanently.
            </p>
          </div>
        </div>
      </div>
...
function P2PPromoBanner({ navigate }) {
  return (
    <div
      ...
      style={{
        background: `linear-gradient(150deg, ${C.forest} 0%, ${C.green} 55%, #0d3d2a 100%)`,
      }}
    >
      {/* decorative glow circles */}
      <div
        className="absolute -top-14 -right-10 w-48 h-48 rounded-full pointer-events-none"
        style={{
      ...
      <div
        ...
        style={{
          background: `radial-gradient(circle, ${C.mint}30 0%, transparent 70%)`,
        }}
      />

      <div className="relative p-4 sm:p-6">
        <span
          className="inline-flex items-center gap-1.5 text-xs font-black px-2.5 py-1 rounded-full mb-2.5"
          style={{
            background: "rgba(244,164,34,0.22)",
            color: "#FCD34D",
        ...
        <p
          className="text-white font-black leading-snug mb-1.5"
        ...
        <p className="text-white/70 text-xs leading-relaxed mb-3 sm:mb-4 max-w-sm">
          Buy or sell Bitcoin &amp; USDT directly with real people. Cash lands
          in your MTN MoMo or bank account in minutes — no waiting.
        </p>
        ...
        <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4 sm:mb-5">
          {[
            ...
            <span
              key={label}
              className="flex items-center gap-1.5 text-xs font-bold px-2 py-1 sm:px-2.5 sm:py-1.5 rounded-xl"
              style={{
...
export default function WalletPage({ user }) {
  ...
  const [displayCurrency, setDisplayCurrency] = useState(
    localStorage.getItem("praqen_currency") || "USD",
  );
  ...
  const [showUsdtHint, setShowUsdtHint] = useState(
    () => !localStorage.getItem("praqen_usdt_hint_seen"),
  );
  const dismissUsdtHint = () => {
    setShowUsdtHint(false);
    localStorage.setItem("praqen_usdt_hint_seen", "1");
  };
  ...
  const loadWallet = async () => {
    try {
      const r = await axios.get(`${API_URL}/hd-wallet/wallet`, {
        headers: authH(),
      });
      setWalletData({
        ...
        available_btc:
          r.data.available_btc != null
            ? r.data.available_btc
            : r.data.balance_btc,
      ...
      if (r.data.btc_price && r.data.btc_price > 0)
        setBtcPrice((p) => (p > 0 ? p : r.data.btc_price));
  ...
  const loadUsdtWallet = async () => {
    ...
    try {
      ...
      toast.error("Failed to load USDT wallet");
    } finally {
  ...
  const loadSwapHistory = async () => {
    try {
      const r = await axios.get(`${API_URL}/swap/history`, {
        headers: authH(),
      });
  ...
  useEffect(() => {
    ...
    const poll = setInterval(async () => {
      ...
      try {
        ...
        /* silent — avoid toast spam on network blip */
      }
    }, 15000);
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (
            t === "wallet" ||
            t === "system" ||
            /received|sent|transfer|deposit/i.test(title)
          ) {
            await loadWallet();
  ...
  const generateAddress = async () => {
    try {
      const r = await axios.post(
        `${API_URL}/hd-wallet/generate-address`,
        {},
        { headers: authH() },
      );
  ...
  const checkDeposit = async () => {
    ...
    try {
      const r = await axios.post(
        `${API_URL}/hd-wallet/check-deposit`,
        {},
        { headers: authH() },
      );
  ...
  const checkUsdtDeposit = async () => {
    ...
    try {
      const r = await axios.get(`${API_URL}/wallet/usdt/check`, {
        headers: authH(),
      });
      toast.success(
        r.data.message || "Deposit scan complete — balance updated",
      );
  ...
  const sendUsdt = async (toAddress, amount, actionCode) => {
    const r = await axios.post(
      `${API_URL}/wallet/usdt/send`,
      { toAddress, amount, actionCode },
      { headers: authH() },
    );
    await loadUsdtWallet();
    return r;
  ...
  const doUsdtInternalTransfer = async ({
    toUsername,
    toTronAddress,
    amountUsdt,
  }) => {
    const r = await axios.post(
      `${API_URL}/wallet/usdt/internal-transfer`,
      { toUsername, toTronAddress, amountUsdt },
      { headers: authH() },
    );
    await loadUsdtWallet();
    return r.data;
  ...
  const doSwap = async () => {
    ...
    try {
      if (swapFrom === "BTC") {
        const r = await axios.post(
          `${API_URL}/swap/btc-to-usdt`,
          { btcAmount: effAmt },
          { headers: authH() },
        );
        toast.success(
          `Swapped ₿${effAmt} BTC → ₮${r.data.to_amount?.toFixed(2)} USDT`,
          { autoClose: 6000 },
        );
        setWalletData((prev) =>
          prev
            ? {
                ...prev,
                balance_btc: r.data.new_btc_balance,
                available_btc: r.data.new_btc_balance,
              }
            : prev,
        );
        setUsdtData((prev) =>
          prev ? { ...prev, balance_usdt: r.data.new_usdt_balance } : prev,
        );
      } else {
        const r = await axios.post(
          `${API_URL}/swap/usdt-to-btc`,
          { usdtAmount: effAmt },
          { headers: authH() },
        );
        toast.success(
          `Swapped ₮${effAmt} USDT → ₿${r.data.to_amount?.toFixed(8)} BTC`,
          { autoClose: 6000 },
        );
        setUsdtData((prev) =>
          prev ? { ...prev, balance_usdt: r.data.new_usdt_balance } : prev,
  ...
  const fxRate =
    displayCurrency === "USD" ? 1 : USD_RATES?.[displayCurrency] || 1;
  const sym = CURRENCY_SYMBOLS[displayCurrency] || `${displayCurrency} `;
  const fmtLocal = (n) =>
    `${sym}${(n * fxRate).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
  ...
  return (
    ...
    >
          ...
          <>
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        <span
                          className="flex items-center gap-1 text-xs font-black px-1.5 py-0.5 rounded-full"
                          style={{
                  ...
                  {showBal ? (
                    <>
                      ...
                      <p
                        ...
                        style={{ fontSize: "clamp(1.35rem, 6vw, 2.5rem)" }}
                      >
                        {fmtLocal(portfolioUsd)}
                      </p>
                      <p className="text-white/70 text-xs sm:text-sm mt-1">
                        Across Bitcoin &amp; USDT wallets
                      </p>

                      {/* Side-by-side asset breakdown — makes both assets visible at once */}
                      <div className="grid grid-cols-2 gap-2 sm:gap-3 mt-3">
                        <div
                          className="rounded-2xl p-2.5 sm:p-3.5 min-w-0"
                          style={{
                        ...
                        >
                          <div className="flex items-center gap-1.5 mb-1">
                            <div
                              ...
                              style={{
                                background:
                                  "linear-gradient(135deg,#F7931A,#e8830a)",
                              }}
                            >
                              <span
                                style={{
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-3">
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-white/80 text-xs font-bold truncate">
                              {fmt(lockedBal, 6)} BTC · {usdtLocked.toFixed(2)}{" "}
                              USDT
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-white/80 text-xs font-bold truncate">
                              {fmt(balance, 6)} BTC ·{" "}
                              {(usdtBal + usdtLocked).toFixed(2)} USDT
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      {btcPrice > 0 && (
                        <p className="text-white/40 text-xs mt-2 truncate">
                          BTC: {fmtLocal(btcPrice)} · USDT: {fmtLocal(1)}
                        </p>
                      )}
                    </>
                  ) : (
            ...
            {showUsdtHint && (
              <div
                ...
                style={{
                  background: `linear-gradient(135deg, #26A17B12, #26A17B08)`,
                  border: "1px solid #26A17B35",
                }}
              >
                <div
                  className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-black" style={{ color: C.g800 }}>
                    You have a USDT wallet too!
                  </p>
                  <p
                    className="text-xs mt-0.5 leading-relaxed"
                    style={{ color: C.g600 }}
                  >
                    Both balances are shown above at a glance. Tap{" "}
                    <strong>"Select Asset ▾"</strong> for USDT's own deposit
                    address &amp; history, or hit Send/Receive/Transfer below
                    and pick USDT from the list.
                  </p>
                </div>
                <button
                  onClick={dismissUsdtHint}
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              {[
                {
                  label: "Available",
                  value: `₿${fmt(availableBal, 6)}`,
                  color: C.gold,
                },
                {
                  label: `${displayCurrency} Value`,
                  value: fmtLocal(availableBal * (btcPrice || 88000)),
                  color: C.success,
                },
                {
                  label: "Transactions",
                ...
                <div
                  key={label}
                  className="bg-white rounded-2xl border p-3 sm:p-4 shadow-sm text-center"
                  style={{ borderColor: C.g200 }}
                >
                  <p
                    className="font-black truncate"
                    style={{ color, fontSize: "clamp(0.75rem, 3vw, 1.25rem)" }}
                  >
                    {value}
                  </p>
                  <p
            ...
            >
              ...
              <div className="space-y-2">
                {[
                  {
                    icon: KeyRound,
                    label: "Self-Custodial HD Wallet",
                    desc: "Your keys derived from master seed — PRAQEN controls nothing",
                  },
                  {
                    icon: ShieldCheck,
                    label: "Escrow Protected Trades",
                    desc: "Trade funds locked until both parties confirm",
                  },
                  {
                    icon: Zap,
                    label: "Auto Deposit Detection",
                    desc: "Balance updates automatically when BTC arrives",
                  },
                  ...
                  >
                    <Icon size={20} className="flex-shrink-0" color={C.green} />
                ...
                {transactions.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-xs mt-1" style={{ color: C.g400 }}>
                      Deposits and trades appear here
                    </p>
                    <button
                      onClick={() => navigate("/buy-bitcoin")}
        ...
        {activeCoin === "USDT" && (
          <>
            ...
            <div
              ...
              style={{
                background: `linear-gradient(145deg, ${C.forest} 0%, #1a5c41 45%, #26A17B 100%)`,
              }}
            >
              {/* Decorative circles */}
              <div
              ...
              />
              <div
                className="absolute -bottom-8 -left-8 w-32 h-32 rounded-full opacity-8"
                style={{
                  background:
                    "radial-gradient(circle, #26A17B 0%, transparent 70%)",
                }}
              />
              ...
              <div className="relative p-3.5 sm:p-6">
                ...
                <div className="flex items-center justify-between mb-3 sm:mb-5 gap-2">
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    {/* Tether logo badge */}
                    <div
                      className="w-9 h-9 sm:w-11 sm:h-11 rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0"
                      style={{
                        background: "linear-gradient(135deg, #26A17B, #1a7a5e)",
                        border: "2px solid rgba(255,255,255,0.25)",
                        boxShadow: "0 4px 16px rgba(38,161,123,0.55)",
                      }}
                    >
                      <svg
                        viewBox="0 0 32 32"
                  ...
                  <div className="flex gap-1.5 flex-shrink-0">
                    ...
                    >
                      <RefreshCw
                        size={12}
                        className={`text-white ${loadingUsdt ? "animate-spin" : ""}`}
                      />
                    </button>
                  </div>
                </div>
                ...
                <div className="mb-3 sm:mb-5">
                  {loadingUsdt && !usdtData ? (
                    ...
                    <>
                      <p className="text-white/55 text-xs font-semibold mb-1 uppercase tracking-widest">
                        Available Balance
                      </p>
                      <div className="flex items-end gap-2">
                        ...
                        <span className="text-white/70 font-black mb-1 text-sm sm:text-base">
                          USDT
                        </span>
                      </div>
                      <p className="text-white/60 text-xs sm:text-sm mt-1.5">
                        ≈ $
                        {(usdtData?.balance_usdt || 0).toLocaleString("en-US", {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}{" "}
                        USD · 1:1 pegged
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              {[
                {
                  label: "USDT Balance",
                  value: `₮${(usdtData?.balance_usdt || 0).toFixed(2)}`,
                  color: C.green,
                  bg: `${C.sage}15`,
                  border: `${C.sage}30`,
                },
                {
                  label: "In Escrow",
                  value: `₮${(usdtData?.locked_balance_usdt || 0).toFixed(2)}`,
                  color: C.warn,
                  bg: `${C.warn}12`,
                  border: `${C.warn}25`,
                },
                ...
                <div
                  key={label}
                  className="rounded-2xl p-3 sm:p-4 text-center shadow-sm"
                  style={{
                    backgroundColor: "#fff",
                ...
                >
                  ...
                  <p
                    className="font-black truncate"
                    style={{ color, fontSize: "clamp(0.7rem, 2.8vw, 1rem)" }}
                  >
                    {value}
                  </p>
                  <p
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p
                        ...
                        style={{ color: "rgba(255,255,255,0.55)" }}
                      >
                        Instant swap · 1% fee · No blockchain delay
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                ...
                <div className="flex items-center justify-center gap-3 mt-4">
                  <div
                    ...
                    style={{
                      ...
                      border: `1px solid ${swapFrom === "BTC" ? "rgba(255,255,255,0.35)" : "rgba(255,255,255,0.12)"}`,
                      transition: "all 0.2s",
                    }}
                  >
                    <div
                    ...
                    style={{
                      background:
                        swapFrom === "USDT"
                          ? "rgba(255,255,255,0.2)"
                          : "rgba(255,255,255,0.08)",
                      border: `1px solid ${swapFrom === "USDT" ? "rgba(255,255,255,0.35)" : "rgba(255,255,255,0.12)"}`,
                      transition: "all 0.2s",
                    }}
                  >
                    <div
              ...
              <div className="bg-white p-5 space-y-4">
                  ...
                  {[
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRight:
                          dir === "BTC" ? `1.5px solid ${C.g200}` : "none",
                        borderRadius:
                          dir === "BTC" ? "14px 0 0 14px" : "0 14px 14px 0",
                      }}
                    >
                      {label}
                    </button>
                ...
                {(() => {
                  ...
                  const effNative =
                    swapInputMode === "usd" && swapRate
                      ? swapFrom === "BTC"
                        ? parseFloat((usdVal / swapRate).toFixed(8))
                        : parseFloat(usdVal.toFixed(2))
                      : natVal;
                  const maxNative =
                    swapFrom === "BTC"
                      ? availableBal
                      : usdtData?.balance_usdt || 0;
                  const maxUsd =
                    swapFrom === "BTC"
                      ? availableBal * (swapRate || 0)
                      : usdtData?.balance_usdt || 0;
                  const minUsd = swapFrom === "BTC" ? 1 : 1; // $1 minimum
                  const hasInput =
                    swapInputMode === "usd" ? usdVal > 0 : natVal > 0;
                  const insufficient =
                    swapInputMode === "usd"
                      ? swapFrom === "BTC"
                        ? effNative > availableBal
                        : usdVal > (usdtData?.balance_usdt || 0)
                      : effNative > maxNative;
                    ...
                    <>
                      <div>
                        ...
                        <div
                          ...
                          style={{
                            background: C.g50,
                            border: `1.5px solid ${insufficient ? "#ef4444" : C.g200}`,
                          }}
                        >
                          <div
                            ...
                            {swapInputMode === "usd" ? (
                              <>
                                <input
                                  ...
                                  value={swapUsdAmount}
                                  onChange={(e) =>
                                    setSwapUsdAmount(e.target.value)
                                  }
                                  placeholder="10.00"
                                  step="0.01"
                                  min="0"
                              ...
                              <>
                                <input
                                  ...
                                  placeholder={
                                    swapFrom === "BTC" ? "0.00000000" : "0.00"
                                  }
                                  step={
                                    swapFrom === "BTC" ? "0.00000001" : "0.01"
                                  }
                                  min="0"
                                  className="w-full text-xl font-black bg-transparent focus:outline-none"
                                  style={{ color: C.g800 }}
                          ...
                          <button
                            onClick={() => {
                              if (swapInputMode === "usd") {
                                setSwapUsdAmount(maxUsd.toFixed(2));
                              } else {
                                setSwapAmount(
                                  swapFrom === "BTC"
                                    ? fmt(availableBal, 8)
                            ...
                            style={{
                              background: `linear-gradient(135deg, ${C.forest}, ${C.green})`,
                              color: "#fff",
                              boxShadow: `0 2px 8px ${C.forest}40`,
                            }}
                          >
                            MAX
                          </button>
                          ...
                          return (
                            <div>
                              ...
                              <div
                                className="flex items-center gap-3 p-4 rounded-2xl"
                                style={{
                                  background: `linear-gradient(135deg, ${C.mist}, #f0fdf6)`,
                              ...
                              >
                                ...
                                <div className="flex-1">
                                  ...
                                  <p
                                    ...
                                    style={{ color: C.green }}
                                  >
                                    {isB2U ? "Tether USDT" : "Bitcoin"} · after
                                    1% fee
                                  </p>
                                </div>
                                <div className="text-right">
                              ...
                              >
                                {[
                                  {
                                    label: isB2U ? "BTC sent" : "USDT sent",
                                    val: `${isB2U ? "₿" : "₮"}${isB2U ? fmt(effNative, 8) : effNative.toFixed(2)}`,
                                  },
                                  {
                                    label: "USD equivalent",
                                    val: `$${sendUsd.toFixed(2)}`,
                                  },
                                  {
                                    label: "Rate",
                                    val: `1 BTC = $${swapRate.toLocaleString("en-US", { maximumFractionDigits: 0 })}`,
                                  },
                                  {
                                    label: "Platform fee (1%)",
                                    val: `${isB2U ? "₮" : "₿"}${isB2U ? fee.toFixed(2) : fee.toFixed(8)}  ≈ $${(fee * (isB2U ? 1 : swapRate)).toFixed(2)}`,
                                  },
                                  {
                                    label: `${isB2U ? "USDT" : "BTC"} received`,
                                    val: `${isB2U ? "₮" : "₿"}${isB2U ? net.toFixed(2) : net.toFixed(8)}`,
                                    bold: true,
                                  },
                                  ...
                                  <div
                                    ...
                                    style={{
                                      backgroundColor: bold ? C.g50 : "#fff",
                                      borderTop:
                                        i > 0 ? `1px solid ${C.g100}` : "none",
                                    }}
                                  >
                                    <span
                                      className={`text-xs ${bold ? "font-black" : "font-semibold"}`}
                                      style={{ color: bold ? C.g700 : C.g500 }}
                                    >
                                      {label}
                                    </span>
                ...
                <div className="flex items-center justify-center gap-4 pt-1">
                  {[
                    ...
                    { icon: Coins, text: "No blockchain fees" },
                  ].map(({ icon: Icon, text }) => (
                    <div key={text} className="flex items-center gap-1">
                      <Icon size={14} className="shrink-0" color={C.g400} />
                      <span
                        className="text-xs font-semibold"
                        style={{ color: C.g400 }}
                      >
              ...
              {swapHistory.length === 0 ? (
                <div className="p-10 text-center">
                  <div
                    ...
                    style={{
                      background: `linear-gradient(135deg, ${C.mist}, #e8f8f0)`,
                      border: `1.5px solid ${C.sage}25`,
                    }}
                  >
                    <ArrowLeftRight size={24} style={{ color: C.sage }} />
                  </div>
                  <p className="font-black text-sm" style={{ color: C.g600 }}>
                    No swaps yet
                  </p>
                  <p className="text-xs mt-1" style={{ color: C.g400 }}>
                ...
                <div>
                  {swapHistory.map((sw, i) => {
                    ...
                    return (
                      <div
                        key={sw.id || i}
                        className="px-5 py-3.5 flex items-center gap-3 hover:bg-gray-50 transition"
                        style={{
                          borderTop: i > 0 ? `1px solid ${C.g100}` : "none",
                        }}
                      >
                        <div
                          ...
                          style={{
                            background: `linear-gradient(135deg, ${C.forest}15, ${C.green}10)`,
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            {isBtcToUsdt ? (
                              <>
                                <span style={{ color: "#F7931A" }}>
                                  ₿{parseFloat(sw.from_amount).toFixed(8)}
                                </span>{" "}
                                →{" "}
                                <span style={{ color: "#26A17B" }}>
                                  ₮{parseFloat(sw.to_amount).toFixed(2)}
                                </span>
                              </>
                            ) : (
                              <>
                                <span style={{ color: "#26A17B" }}>
                                  ₮{parseFloat(sw.from_amount).toFixed(2)}
                                </span>{" "}
                                →{" "}
                                <span style={{ color: "#F7931A" }}>
                                  ₿{parseFloat(sw.to_amount).toFixed(8)}
                                </span>
                              </>
                            )}
                          </p>
                          ...
                          >
                            @ $
                            {parseFloat(sw.rate).toLocaleString("en-US", {
                              maximumFractionDigits: 0,
                            })}
                            /BTC · fee{" "}
                            {isBtcToUsdt
                              ? `₮${parseFloat(sw.fee_amount).toFixed(2)}`
                              : `₿${parseFloat(sw.fee_amount).toFixed(8)}`}
                          </p>
                        </div>
                        <div className="text-right flex-shrink-0">
                          <span
        ...
        <div className="max-w-2xl mx-auto px-4 pt-8 pb-5">
          <div className="grid grid-cols-2 gap-8 mb-6">
            <div>
              ...
              <p
                className="text-xs leading-relaxed my-3"
      ...
      {showUsdtSend && (
        <UsdtWithdrawModal
          balance={usdtData?.balance_usdt || 0}
          onClose={() => setShowUsdtSend(false)}
          onSend={sendUsdt}
          kycStatus={userVerif}
        />
      )}
      {showUsdtInternal && (
        <UsdtInternalTransferModal
          balance={usdtData?.balance_usdt || 0}
          onClose={() => setShowUsdtInternal(false)}
          onTransfer={doUsdtInternalTransfer}
        />
      )}
      {showRecv && (
        <ReceiveModal
      ...
      {assetPicker && (
        <AssetPickerSheet
          title={
            assetPicker.type === "send"
              ? "Send"
              : assetPicker.type === "receive"
                ? "Deposit"
                : "Transfer"
          }
      ...
      {showInternal && (
        <InternalTransferModal
          ...
          onDone={(newBal) => {
            setWalletData((prev) =>
              prev
                ? { ...prev, available_btc: newBal, balance_btc: newBal }
                : prev,
            );
            setShowInternal(false);
            setRepeatUsername(null);
