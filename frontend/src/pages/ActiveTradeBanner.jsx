...
const isGiftTrade = (t) =>
  !!(
    t.trade_type?.toUpperCase().includes("GIFT") ||
    t.listing?.listing_type?.toUpperCase().includes("GIFT")
  );
const tradeColor = (isGift, isBuyer) =>
  isGift ? "#8B5CF6" : isBuyer ? "#F59E0B" : "#2D6A4F";
...
function TradeCard({ trade, user, onClose, onExpire }) {
  ...
  const cpName =
    cp.username || (isBuyer ? trade.seller_name : trade.buyer_name) || "—";
  ...
  const actionInfo = isBuyer
    ? isPaid
      ? {
          icon: <CheckCircle size={16} />,
          message: `Payment sent — awaiting release from ${cpName}`,
        }
      : {
          icon: <HandCoins size={16} />,
          message: `Send ${payDisp} via ${trade.payment_method || "—"} to ${cpName}`,
        }
    : isPaid
      ? {
          icon: <Unlock size={16} />,
          message: `${cpName} paid! Check ${trade.payment_method || "—"} and RELEASE BITCOIN`,
        }
      : {
          icon: <Clock3 size={16} />,
          message: `Waiting for ${cpName} to send payment…`,
        };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {timeLeft !== null && !isPaid && (
            <span
              className={`font-mono text-xs font-black px-2 py-0.5 rounded-full ${isUrgent ? "animate-pulse" : ""}`}
              style={{
                backgroundColor: isUrgent ? C.danger : typeColor,
                color: "#fff",
              }}
            >
      ...
      <div
        ...
        style={{ borderColor: C.g100, backgroundColor: "#fffef9" }}
      >
        <p
      ...
      {isUrgent && !isPaid && (
        <div
          className="px-4 py-2 border-t animate-pulse"
          style={{
        ...
        >
          ...
          >
            ...
            Under 5 min left! Trade cancels at 00:00 — BTC returns to wallet
            instantly.
          </p>
        </div>
      )}
