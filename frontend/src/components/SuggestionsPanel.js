...
export default function SuggestionsPanel({ user }) {
  ...
  const supportFallback = (message, currentAiMsgs) => {
    ...
    if (
      /^(hey|hi|hello|good\s*(morning|afternoon|evening)|howdy|yo|hiya|sup)\b/.test(
        q,
      )
    ) {
      const greetings = [
  ...
  const fetchAI = async (
    ...
    try {
      ...
      setAiMsgs((prev) => {
        ...
        const isDuplicate =
          replyText && prev.some((m) => m.text.trim() === replyText);
        const finalText =
          isDuplicate || !replyText
      ...
      setAiMsgs((prev) => [
        ...prev,
        { role: "ai", text: supportFallback(message, prev) },
      ]);
    } finally {
      setAiLoad(false);
  ...
  const createTicket = async () => {
    ...
    try {
      ...
      const msgsRes = await axios.get(
        `${API_URL}/support/tickets/${newTicket.id}/messages`,
        { headers: authH() },
      );
      ...
      toast.error(
        e.response?.data?.error || "Failed to create ticket. Please try again.",
      );
    } finally {
      setCreating(false);
    }
  ...
  const sendReply = async () => {
    ...
    try {
      const r = await axios.post(
        `${API_URL}/support/tickets/${ticket.id}/messages`,
        { message: text },
        { headers: authH() },
      );
      ...
      setReplyText(text);
      toast.error(
        e.response?.data?.error || "Failed to send. Please try again.",
      );
    } finally {
      setReplying(false);
    }
  ...
  const submitSuggestion = async () => {
    ...
    try {
      ...
      toast.error(
        e.response?.data?.error || "Failed to submit. Please try again.",
      );
    } finally {
      setSugPost(false);
    }
  ...
  const headerCfg = {
    home: {
      title: "PRAQEN Support",
      sub: "How can we help you today?",
      icon: <MessageCircle size={18} color="white" />,
      step: null,
    },
    "topic-selected": {
      title: topic?.label || "Support",
      sub: "Step 1 of 2 — Review your topic",
      icon: topic?.icon ? (
        <topic.icon size={16} color="white" />
      ) : (
    ...
    "ticket-form": {
      ...
      sub: "Step 2 of 2 — Tell us what's happening",
      icon: <Ticket size={18} color="white" />,
      step: [2, 2],
    },
    "ticket-created": {
      title: "Ticket Created!",
      sub: `Ticket #${ticket?.id?.slice(0, 8).toUpperCase() || "…"}`,
    ...
    suggest: {
      title: "Drop a Suggestion",
      sub: "We read every single one",
      icon: <Lightbulb size={18} color="white" />,
      step: null,
    },
  };
  ...
  const canGoBack =
    mode !== "home" && mode !== "chat" && mode !== "ticket-created";
          ...
          >
            ...
            >
              ...
              <div
                ...
                style={{ minHeight: isMobile ? 60 : 56 }}
              >
                {canGoBack && (
                  <button
                    onClick={() => {
              ...
              >
                ...
                <div className="px-4 pt-4 pb-3">
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-black text-white leading-none inline-flex items-center gap-1.5">
                        Hi
                        {user
                ...
                <div className="px-4 pb-3">
                  ...
                  >
                    {TOPICS.map((t) => (
                      ...
                      >
                        <t.icon size={isMobile ? 22 : 18} color={t.color} />
                        <span
                          style={{
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px]" style={{ color: "#94A3B8" }}>
                        Share ideas · Our team reads them all
                      </p>
                    </div>
                ...
                <div className="mx-4 mb-4 flex gap-2">
                  {[
                    {
                      icon: <Shield size={11} color="#1B4332" />,
                      text: "Secure & Private",
                    },
                    {
                      icon: <Clock size={11} color="#1B4332" />,
                      text: "Fast Response",
                    },
                    {
                      icon: <Headphones size={11} color="#1B4332" />,
                      text: "Human + AI Support",
                    },
                  ].map((b) => (
                    <div
                      key={b.text}
            ...
            {mode === "topic-selected" && (
              ...
              >
                ...
                <div
                  ...
                  style={{
                    backgroundColor: topic?.bg || "#F0FDF4",
                    border: `2px solid ${topic?.color || "#1B4332"}25`,
                  }}
                >
                  <div className="flex items-center gap-3 mb-2">
                    {topic?.icon && (
                      <topic.icon size={26} color={topic?.color} />
                    )}
                    <div>
                      <p
                ...
                >
                  ...
                  {[
                    {
                      n: "1",
                      text: "You describe your issue in the next step",
                    },
                    {
                      n: "2",
                      text: "A ticket is created instantly with a unique ID",
                    },
                    {
                      n: "3",
                      text: "Our AI responds immediately with helpful info",
                    },
                    {
                      n: "4",
                      text: "A human support agent follows up in the chat",
                    },
                  ].map((s) => (
                    <div key={s.n} className="flex items-start gap-3">
                      <span
                        className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-black text-white"
                        style={{ backgroundColor: "#1B4332", marginTop: 1 }}
                      >
                        {s.n}
                      </span>
                      <p
                ...
                {user ? (
                  ...
                  >
                    ...
                    <span
                      className="text-[10px] font-black px-2 py-0.5 rounded-full flex-shrink-0 inline-flex items-center gap-1"
                ...
                <p
                  ...
                  style={{ color: "#CBD5E1" }}
                >
                  Your account info is sent with the ticket so we can help
                  faster
                </p>
              </div>
            )}
            ...
            {mode === "ticket-form" && (
              ...
              >
                ...
                <div
                  ...
                  style={{
                    backgroundColor: topic?.bg || "#F0FDF4",
                    border: `1.5px solid ${topic?.color || "#1B4332"}20`,
                  }}
                >
                  {topic?.icon && <topic.icon size={15} color={topic?.color} />}
                  <p
                    className="text-xs font-black"
                    style={{ color: topic?.color }}
                  >
                ...
                <div>
                  <label
                    className="text-[11px] font-black uppercase tracking-wider block mb-1.5"
                    style={{ color: "#64748B" }}
                  >
                    Subject
                  </label>
                  <input
                    ...
                    style={{
                      borderColor: subject ? "#1B4332" : "#E2E8F0",
                      backgroundColor: "#fff",
                      fontSize: 16,
                      fontFamily: "inherit",
                    }}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    style={{
                      borderColor: msgBody ? "#1B4332" : "#E2E8F0",
                      backgroundColor: "#fff",
                      fontSize: 16,
                      fontFamily: "inherit",
                    }}
                ...
                <button
                  ...
                  disabled={creating || !msgBody.trim()}
                  className="w-full py-3.5 rounded-xl font-black text-sm text-white flex items-center justify-center gap-2 transition hover:opacity-90 disabled:opacity-40"
                  style={{
                    background: "linear-gradient(135deg,#1B4332,#2D6A4F)",
                  }}
                >
                  {creating ? (
                    ...
                    <>
                      <Ticket size={14} /> Create Ticket &amp; Open Chat{" "}
                      <ArrowRight size={13} />
                    </>
                  )}
                </button>
                ...
                <p
                  ...
                  style={{ color: "#CBD5E1" }}
                >
                  By submitting you agree to our support terms · Response time:
                  &lt; 24 hours
                </p>
              </div>
            )}
              ...
              >
                ...
                <div className="w-full space-y-2">
                  {[
                    {
                      icon: CheckCircle,
                      text: "Ticket submitted to our support team",
                    },
                    {
                      icon: Bot,
                      text: "PRAQEN AI is typing a quick response…",
                    },
                    {
                      icon: UserCheck,
                      text: "A human agent will reply within 24 hours",
                    },
                  ].map((s) => (
                    <div
                      key={s.text}
              ...
              <>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <p
                        ...
                        style={{ color: "#64748B" }}
                      >
                        {topic?.icon && <topic.icon size={10} />} {topic?.label}{" "}
                        · AI + Human support · Ticket #
                        {ticket?.id?.slice(0, 8).toUpperCase()}
                      </p>
                    </div>
                  </div>
                ...
                >
                  ...
                  <div className="flex flex-col items-center gap-1 py-2 text-center">
                    <span
                      className="text-[10px] font-bold px-3 py-1 rounded-full inline-flex items-center gap-1"
                      style={{ backgroundColor: "#D1FAE5", color: "#1B4332" }}
                    >
                      <CheckCircle size={10} />
                      Ticket #{ticket?.id?.slice(0, 8).toUpperCase()} · Open
                ...
                >
                  ...
                  <button
                    onClick={sendReply}
                    disabled={!replyText.trim() || replying || aiLoading}
                    style={{
                      ...
                      justifyContent: "center",
                      opacity:
                        !replyText.trim() || replying || aiLoading ? 0.4 : 1,
                      WebkitTapHighlightColor: "transparent",
                    }}
                  >
                    {replying ? (
              ...
              >
                {sugDone ? (
                  <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-2">
                    ...
                    <div>
                      <p
                        className="font-black text-base inline-flex items-center gap-1.5"
                        style={{ color: "#92400E" }}
                      >
                        Thank you! <HeartHandshake size={16} />
                      </p>
                      <p
                        className="text-sm mt-1 leading-relaxed"
                        style={{ color: "#64748B" }}
                      >
                        Your suggestion is sent to the PRAQEN team. We read
                        every single one!
                      </p>
                    </div>
                    <button
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        style={{
                          borderColor: sugTitle ? "#D97706" : "#E2E8F0",
                          backgroundColor: "#fff",
                        }}
                      />
                    </div>
                    <div>
                      ...
                      <textarea
                        ...
                        style={{
                          borderColor: sugBody ? "#D97706" : "#E2E8F0",
                          backgroundColor: "#fff",
                        }}
                      />
                    </div>
                    <button
                      ...
                      disabled={sugPosting || !sugTitle.trim()}
