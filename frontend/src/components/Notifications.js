...
const flag = (c) =>
  !c || c.length !== 2
    ...
    : c
        .toUpperCase()
        .replace(/./g, (ch) =>
          String.fromCodePoint(0x1f1e0 + ch.charCodeAt(0) - 65),
        );
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 16,
        margin: "0 0 10px",
        border: `1px solid ${n.is_read ? "#f3f4e8" : "#CBD5E1"}`,
        boxShadow: shadow,
        cursor: "pointer",
        overflow: "hidden",
      ...
      onMouseEnter={(e) => {
        e.currentTarget.style.boxShadow = shadowH;
...
function getRealTradeRef(n) {
  ...
  if (n.trade?.id)
    return String(n.trade.id).replace(/-/g, "").slice(0, 8).toUpperCase();
  const combined = (n.title || "") + " " + (n.message || "");
  const m =
    combined.match(/#([A-Za-z0-9]{4,12})/) ||
    combined.match(/trade[^\w]([A-Za-z0-9]{4,12})/i);
    ...
    <NCard n={n} onNavigate={onNavigate}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          ...
          <span style={{ fontSize: 12, color: T.g400, fontWeight: 600 }}>
            {tradeTimeStr(n.created_at)}
          </span>
          {!n.is_read && (
            <span
              style={{
      ...
      >
        ...
        <span style={{ fontSize: 13, color: T.g500, fontWeight: 600 }}>
          {isAnon ? "—" : displayName}
        </span>
      </div>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            {isAnon
              ? "Browsed your profile anonymously"
    ...
    <NCard n={n} onNavigate={onNavigate}>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            {isAnon
              ? "Browsed your offer anonymously"
              : "Might be interested in your offer"}
          </p>
        </div>
        {!isAnon && (
          <span
...
function TradeNotifCard({ n, trade, userId, onNavigate, isChat = false }) {
  ...
  const msgActorName = !cpRaw
    ? (n.message || "").match(
        /^([A-Za-z0-9_]+)\s+(?:wants to|cancelled|canceled|paid|disputed|completed)/i,
      )?.[1] || (n.message || "").match(/\bwith\s+([A-Za-z0-9_]+)\b/i)?.[1]
    : null;
  ...
  const GIFT_BRANDS =
    /amazon|itunes|apple|google.?play|steam|walmart|ebay|target|playstation|xbox|netflix|spotify|visa gift|mastercard gift|best buy/i;
  const isGiftCard =
    /gift/i.test(trade.trade_type || "") || GIFT_BRANDS.test(pm);
          ...
          {isGiftCard ? (
            ...
            <div
              style={{
                ...
                height: 22,
                borderRadius: "50%",
                background: "linear-gradient(135deg,#F7931A,#E8790A)",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                flexShrink: 0,
                boxShadow: "0 1px 4px rgba(247,147,26,0.4)",
              }}
            >
...
function BasicCard({ n, userId, onNavigate }) {
  ...
  const isExpired =
    /expir/i.test(title) || /expir/i.test(type) || /expir/i.test(msg);
  const isNewTrade =
    /new trade|trade request/i.test(title) ||
    /trade_new|new_trade/i.test(type) ||
    /wants to (buy|sell)/i.test(msg);
  const isDispute = /disput/i.test(title) || /disput/i.test(type);
  const isPayment = /payment|paid/i.test(title);
  const isCompleted =
    /complet/i.test(title) ||
    /complet/i.test(type) ||
    type === "trade_done" ||
    type === "trade_complete" ||
  ...
  if (isTradeRelated) {
    ...
    const fiatM = msg.match(
      /([\d,]+(?:\.\d+)?)\s*(GHS|NGN|KES|ZAR|USD|GBP|EUR|UGX|TZS|XAF|XOF)\b/i,
    );
    ...
    const GIFT_BRANDS_RE =
      /amazon|itunes|apple|google.?play|steam|walmart|ebay|target|playstation|xbox|netflix|spotify|visa gift|mastercard gift|best buy/i;
    const basicIsGiftCard =
      GIFT_BRANDS_RE.test(parsedPm) || /gift.?card/i.test(msg);
    ...
    if (basicIsGiftCard) {
      ...
      else if (/\bbuy\b/i.test(type) && !/sell/i.test(type))
        dirLabel = "Buy BTC";
      else if (/\bsell\b/i.test(type) && !/buy/i.test(type))
        dirLabel = "Sell BTC";
    }
    ...
    const parsedActorName = !vendor
      ...
      : null;
    const displayActor =
      vendor || (parsedActorName ? { username: parsedActorName } : null);
      ...
      <NCard n={n} onNavigate={onNavigate}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            ...
            {basicIsGiftCard ? (
              ...
              <div
                style={{
                  ...
                  justifyContent: "center",
                  flexShrink: 0,
                  boxShadow: "0 1px 4px rgba(247,147,26,0.35)",
                }}
              >
                <span style={{ fontSize: 11, color: "#fff", fontWeight: 900 }}>
                  ₿
            ...
            <span style={{ fontSize: 12, color: "#ffffffd9", fontWeight: 600 }}>
              {tradeTimeStr(n.created_at)}
            </span>
            {!n.is_read && (
              <span
          ...
          {displayActor ? (
            ...
            >
              ...
              <div>
                ...
                {displayActor.country && (
                  <span style={{ fontSize: 13, lineHeight: 1 }}>
                    {flag(displayActor.country)}
                  </span>
                )}
              </div>
            </div>
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ margin: "3px 0 0", fontSize: 11, fontWeight: 500 }}>
          ...
          {joinStr && (
            <span style={{ color: T.g400 }}> · Joined {joinStr}</span>
          )}
        </p>
      </div>
      <button
...
const dedupByTrade = (list) => {
  ...
  list.forEach((n) => {
    ...
    if (!groups.has(tid)) {
      groups.set(tid, {
        best: n,
        hasUnread: isMsg && !n.is_read,
        count: isMsg && !n.is_read ? 1 : 0,
      });
    } else {
...
function NotifCard({ n, userId, onNavigate }) {
  ...
  useEffect(() => {
    ...
    const uuidMatch =
      n.action?.match(UUID_RE) || n.data?.trade_id?.match(UUID_RE);
    const uuid = uuidMatch?.[0] || n.data?.trade_id;
    if (!uuid) return;
    const token = localStorage.getItem("token");
    fetch(`${API_URL}/trades/${uuid}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (d?.trade || d?.id) setTrade(d.trade || d);
      })
      .catch(() => {});
  ...
  if (
    type === "offer_view" ||
    /viewed your offer/i.test((n.title || "") + (n.message || ""))
  ) {
...
function matchFilter(n, filter) {
  ...
  if (filter === "views") {
    return (
      /profile_view|offer_view/i.test(type) || /viewed your/i.test(title + msg)
    );
  }

  const isReferral =
    type === "referral" ||
    /referral|commission|affiliate/i.test(type + title + msg);

  if (filter === "referral") return isReferral;

  if (filter === "system") {
    if (isReferral) return false;
    return /update|broadcast|system|welcome|bonus|received|sent|transfer/i.test(
      type + title + msg,
    );
  }
}
...
export default function Notifications({ user }) {
  ...
  useEffect(() => {
    ...
    return () => {
      vp.removeEventListener("resize", onVPChange);
      vp.removeEventListener("scroll", onVPChange);
    };
  }, []);
  ...
  const pendingReadRef = useRef(new Set()); // ids marked read locally, not yet confirmed
  const pendingAllReadAt = useRef(0);

  const load = async () => {
    if (!user) return;
    setLoading(true);
    try {
      const r = await axios.get(`${API_URL}/notifications`, {
        headers: hdrs(),
        timeout: 10000,
      });
      errCountRef.current = 0;

      let incoming = r.data.notifications || [];

      // ── Reconcile against optimistic local state so a stale/late GET
      //    can't undo a read-state the user already triggered ──
      incoming = incoming.map((n) => {
        if (pendingReadRef.current.has(n.id)) return { ...n, is_read: true };
        if (
          pendingAllReadAt.current &&
          new Date(n.created_at).getTime() < pendingAllReadAt.current
        ) {
      ...
      if (seenIdsRef.current === null) {
        ...
        incoming
          .filter(
            (n) => !n.is_read && new Date(n.created_at).getTime() > cutoff,
          )
          .slice(0, 3)
          .forEach(showToast);
      } else {
  ...
  const markRead = async (id) => {
    ...
    try {
      ...
      // failed — let it re-sync as unread on next load
  ...
  const markAllRead = async () => {
    pendingAllReadAt.current = Date.now();
  ...
  const loadChatMsgs = async (uid) => {
    try {
      const r = await axios.get(`${API_URL}/referral-messages/${uid}`, {
        headers: hdrs(),
      });
  ...
  const sendChatMsg = async () => {
    ...
    try {
      await axios.post(
        `${API_URL}/referral-messages/${chatRef.id}`,
        { message: text },
        { headers: hdrs() },
      );
  ...
  const referralNotificationCount = dedupByTrade(
    notifs.filter((n) => matchFilter(n, "referral")),
  ).length;

  const referralCount =
    referrals.length + (myReferrer ? 1 : 0) + referralNotificationCount;
  if (!user) return null;
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              {unread > 0 && (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 9,
                    padding: isMobile ? "7px 12px" : "4px 9px",
                    cursor: "pointer",
                  }}
                >
              ...
              >
                <X size={isMobile ? 17 : 14} color="#fff" />
              </button>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: isMobile ? 6 : 4 }}>
            {tabDefs.map((f) => {
              const count =
                f.id === "all"
                  ? unread
                  : notifs.filter((n) => !n.is_read && matchFilter(n, f.id))
                      .length;
              ...
              return (
                <button
                  key={f.id}
                  onClick={() => {
                    if (f.id !== "referral") setChatRef(null);
                    setFilter(f.id);
                  }}
                  style={{
                    flex: 1,
        ...
        <>
              ...
              >
                <div
                  style={{
                    ...
                    height: 4,
                    borderRadius: 99,
                    backgroundColor: "rgba(131, 17, 17, 0.3)",
                  }}
                />
              </div>
            )}
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                >
                  ...
                  {chatRef.country && (
                    <span
                      style={{ marginLeft: 6, fontSize: isMobile ? 14 : 12 }}
                    >
                      {flag(chatRef.country)}
                    </span>
                  )}
                </p>
                <p
                ...
                >
                  ...
                  />
                  {chatRef.trade_count > 0
                    ? `${chatRef.trade_count} trade${chatRef.trade_count !== 1 ? "s" : ""}`
                    : "No trades yet"}{" "}
                  · Referral chat
                </p>
              </div>
          ...
          <div
            ...
            style={{
              ...
              padding: isMobile ? "14px 14px 6px" : "12px 12px 4px",
          ...
          >
            {chatMsgs.length === 0 && (
              ...
              >
                ...
                >
                  Say hi to <strong>{chatRef.username}</strong> and encourage
                  them to trade!
                </p>
              </div>
            )}
            ...
            refLoading ? (
              ...
              >
                <div
                  style={{
                    ...
                    height: 30,
                    borderRadius: "50%",
                    border: `2.5px solid ${T.mint}`,
                    borderTopColor: "transparent",
                    animation: "notif-spin 0.8s linear infinite",
                    marginBottom: 12,
                  }}
                />
                <p style={{ fontSize: 13, color: T.g400, fontWeight: 600 }}>
                  Loading…
                </p>
              ...
              >
                ...
                >
                  ...
                  and start trading on PRAQEN.
                </p>
              </div>
            ) : (
              <>
                {/* Referral notification cards */}
        ...
        >
          {filter === "referral" ? (
            ...
            >
              ...
              {filter !== "all" && (
                <span style={{ color: "#000000a7" }}>
                  {" "}
                  · {tabDefs.find((f) => f.id === filter)?.label}
                </span>
              )}
            </p>
          ) : (
            ...
            {toasts.map((t) => (
              <div
                ...
                style={{
                  ...
                  animation: "prqSlideDown 0.3s ease",
                }}
              >
                <div
                  style={{
                    height: 4,
                    background: `linear-gradient(90deg,${T.forest},${T.mint})`,
                  }}
                />
                <div
                  style={{
