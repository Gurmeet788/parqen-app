...
function getMarketInfo(listingType) {
  ...
  if (t === "SELL")
    return { label: "SELL TRADE", color: "#D97706", bg: "#FEF3C7" };
  if (t === "BUY")
    return { label: "BUY TRADE", color: "#1B4332", bg: "#DCFCE7" };
  if (t === "BUY_GIFT_CARD" || t === "SELL_GIFT_CARD")
    return { label: "GIFT CARD", color: "#0D9488", bg: "#CCFBF1" };
...
function resolveExpiresAt(expiresAt, createdAt, limitMins) {
  ...
  const computed = createdAt
    ? new Date(
        new Date(createdAt).getTime() + (limitMins || 30) * 60 * 1000,
      ).toISOString()
    : null;
...
function TradeTimer({ expiresAt, timeLimitMins = 30, onExpire }) {
  ...
  return (
    <div className="flex flex-col items-end gap-1 flex-shrink-0">
      <span
        className={`inline-flex items-center gap-1 text-xs font-mono font-black px-2.5 py-1 rounded-full ${urgent ? "animate-pulse" : ""}`}
...
function TraderPopup({ cpId, cpFallback, onClose }) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_URL}/users/${cpId}`)
      .then((r) => {
        if (r.data?.user || r.data) setUser(r.data.user || r.data);
      })
      .catch(() => {})
  ...
  const trust =
    total > 0
      ...
      : parseInt(user.total_trades || 0) > 0
        ? 100
        : 0;
            ...
            <div className="min-w-0">
              ...
              {loading ? (
                ...
                >
                  <span
                    style={{
                      color: badge.iconColor || badge.textColor,
                      fontSize: "13px",
                    }}
                  >
        ...
        <div className="grid grid-cols-3 gap-2 p-4">
          {[
            { label: "Trades", value: trades, color: "#1B4332" },
            {
              ...
              value: `${trust}%`,
              color:
                trust >= 80 ? "#16A34A" : trust >= 50 ? "#D97706" : "#DC2626",
            },
            {
              label: "Completion",
              value: completion > 0 ? `${Math.round(completion)}%` : "—",
              color: "#2563EB",
            },
...
export default function ActiveTradeCard({ trade, onExpire, pageColor }) {
  ...
  const cp =
    (isBuyer
      ? typeof trade.seller === "object"
        ? trade.seller
        : {}
      : typeof trade.buyer === "object"
        ? trade.buyer
        : {}) || {};
  ...
  const listingType =
    trade.listing?.listing_type || trade.listing_type || trade.trade_type || "";
  const market = listingType
    ? getMarketInfo(listingType)
    : {
        label: isBuyer ? "BUY TRADE" : "SELL TRADE",
        color: isBuyer ? "#1B4332" : "#D97706",
        bg: isBuyer ? "#DCFCE7" : "#FEF3C7",
      };
  const timeLimitMins = sanitizeLimitMins(
    trade.listing?.time_limit || trade.time_limit || 30,
  );

  // Timer: use server expires_at, fall back to created_at + limit
  const effectiveExpiresAt = resolveExpiresAt(
    trade.expires_at,
    trade.created_at,
    timeLimitMins,
  );
  ...
  const localAmt =
    fmtLocal(sym, trade.amount_local) || fmtLocal("$", trade.amount_usd);
  const btcAmt = fmtBtc(trade.amount_btc);

  const youPayAmt = isBuyer ? localAmt : btcAmt;
  const youRecvAmt = isBuyer ? btcAmt : localAmt;
  const youPayNote = isBuyer
    ? `via ${trade.payment_method || trade.listing?.payment_method || "—"}`
    : "Bitcoin escrow";
  const youRecvNote = isBuyer
    ? "Bitcoin"
    : `via ${trade.payment_method || trade.listing?.payment_method || "—"}`;
  ...
  return (
    <>
      ...
      <div
        className="rounded-2xl mb-3 overflow-hidden w-full"
        style={{
          background: "#FFFFFF",
          border:
            trade.status === "PAYMENT_SENT"
              ? "1.5px solid #2563EB50"
              : `1.5px solid ${cfg.statusColor}30`,
          borderLeft:
            trade.status === "PAYMENT_SENT"
              ? "4px solid #2563EB"
              : `4px solid ${btnColor}`,
          ...
          animation:
            trade.status === "PAYMENT_SENT"
              ? "pmtBorderPulse 2.5s ease-in-out infinite"
              : undefined,
        }}
      >
        ...
        <div className="flex items-center justify-between px-4 pt-3 pb-1 gap-2">
          <span
            className="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full flex-shrink-0"
        ...
        <div className="flex items-center justify-between px-4 pb-3 gap-2 flex-wrap">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            >
              ...
              <span style={{ color: badge.textColor }} className="ml-0.5">
                {badge.label}
              </span>
            </span>
          </div>
        ...
        <div
          className="mx-4 mb-3"
          style={{ height: "1px", backgroundColor: "#F1F5F9" }}
        />
        ...
        {trade.status === "PAYMENT_SENT" && (
          ...
          >
            ...
            <p
              ...
              style={{ color: "#1E40AF" }}
            >
              Payment sent — awaiting Bitcoin release from{" "}
              {cp.username || "seller"}
            </p>
          </div>
        )}
        ...
        <div className="px-3 pb-3 flex items-stretch gap-1.5">
          <div
            className="flex-1 min-w-0 rounded-xl px-2.5 py-2.5"
          ...
          <div
            className="flex-1 min-w-0 rounded-xl px-2.5 py-2.5"
            style={{
              backgroundColor: `${btnColor}10`,
              border: `1px solid ${btnColor}25`,
            }}
          >
            <p
              className="text-[9px] font-black uppercase tracking-widest mb-1"
              style={{ color: btnColor, opacity: 0.7 }}
            >
              You Receive
            </p>
            <p
              className="font-black text-sm leading-tight break-all"
            ...
            <p
              ...
              style={{ color: btnColor, opacity: 0.6 }}
            >
              {youRecvNote}
            </p>
          </div>
        ...
        <button
          ...
          style={{
            backgroundColor:
              trade.status === "PAYMENT_SENT" ? "#2563EB" : btnColor,
          }}
        >
